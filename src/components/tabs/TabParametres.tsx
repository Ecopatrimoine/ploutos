...
// ─── Effacement RGPD (C3-b) — libellés stables pour la liste des demandes ────
type DeletionRequest = {
  id: string;
  scope: "cabinet" | "dossier";
  status: string;
...
const DELETION_STATUS_LABEL: Record<string, string> = {
  ...
  export_remis: "Export remis",
  grace: "Délai d'annulation en cours",
};
...
const TabParametres = React.memo(function TabParametres(props: any) {
  ...
  // ─── Effacement RGPD (C3-b → EF deletion-request) ────────────────────────
  // Liste des demandes actives (SELECT direct, RLS filtre au CGP connecté) +
  // demande cabinet (mot EFFACER) + annulation. Même patron d'appel EF.
  ...
  const hasActiveCabinetRequest = requests.some(r => r.scope === "cabinet");

  const loadRequests = async () => {
    try {
      const { data, error } = await supabase
        .from("deletion_requests")
        .select("id, scope, status, grace_ends_at, target_client_id, requested_at")
        .in("status", ["demande", "export_remis", "grace"])
        .order("requested_at", { ascending: false });
      if (error) throw error;
      setRequests((data ?? []) as DeletionRequest[]);
    } catch (e) {
      console.error("Demandes d'effacement — lecture échouée:", e);
      setReqError("Impossible de charger les demandes d'effacement en cours.");
    }
  };

  useEffect(() => { loadRequests(); }, []);

  const handleEraseCabinet = async () => {
    if (eraseWord.trim() !== "EFFACER") return;
    setErasing(true);
    setEraseError(null);
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const token = session?.access_token;
      if (!token) {
        setEraseError("Votre session a expiré. Reconnectez-vous puis réessayez.");
        return;
      }
      const res = await fetch(`${SUPABASE_FUNCTIONS_URL}/deletion-request`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
        body: JSON.stringify({ action: "create", scope: "cabinet" }),
      });
      ...
      if (res.ok && data.id) {
        ...
        console.error("Demande d'effacement échouée:", data.error ?? res.status);
        setEraseError(data.error || "La demande d'effacement a échoué. Réessayez dans un instant.");
      }
    } catch (e) {
      console.error("Demande d'effacement — échec réseau:", e);
      setEraseError("Connexion impossible. Vérifiez votre accès internet puis réessayez.");
    } finally {
      setErasing(false);
    }
  };
  ...
  const handleCancelRequest = async (requestId: string) => {
    ...
    try {
      ...
      const token = session?.access_token;
      if (!token) {
        setReqError("Votre session a expiré. Reconnectez-vous puis réessayez.");
        return;
      }
      const res = await fetch(`${SUPABASE_FUNCTIONS_URL}/deletion-request`, {
        method: "POST",
      ...
      if (res.ok && data.status === "annule") {
        await loadRequests();
      } else {
        console.error("Annulation d'effacement échouée:", data.error ?? res.status);
        setReqError(data.error || "L'annulation a échoué. Réessayez dans un instant.");
      }
    } catch (e) {
      console.error("Annulation d'effacement — échec réseau:", e);
      setReqError("Connexion impossible. Vérifiez votre accès internet puis réessayez.");
    } finally {
    ...
    <TabsContent value="parametres" className="space-y-6">
      ...
      <Dialog open={confirmOpen} onOpenChange={(o) => { if (!o) { setConfirmOpen(false); setEraseError(null); } }}>
        <DialogContent aria-describedby="erase-desc" className="max-w-md rounded-2xl" style={{ background: SURFACE.card, border: `1px solid ${SURFACE.border}` }}>
          ...
          <div id="erase-desc" className="text-sm text-slate-600 leading-relaxed space-y-2">
            <p>Cette demande déclenche l'effacement <strong>définitif</strong> de l'intégralité de vos données : dossiers clients, documents, paramètres du cabinet, CRM et commissions.</p>
          ...
          <div>
            <Label className="text-xs font-bold tracking-wide mb-1.5 block text-slate-600">
              Pour confirmer, saisissez <span className="font-mono text-red-700">EFFACER</span>
            </Label>
            <Input
              value={eraseWord}
          ...
          <div className="flex justify-end gap-2">
            <button
              onClick={() => { setConfirmOpen(false); setEraseError(null); }}
            ...
            <button
              onClick={handleEraseCabinet}
              disabled={erasing || eraseWord.trim() !== "EFFACER"}
            ...
            >
              {erasing
                ...
                : <><Trash2 className="h-3.5 w-3.5" aria-hidden="true" /> Demander l'effacement</>}
            </button>
          </div>
        </DialogContent>
      </Dialog>
